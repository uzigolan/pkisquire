...
- Interactive HTML view is generated from `security/pip-audit.json` and enriches CVEs with NVD first, then GHSA, then OSV for severity/score when available.
- Interactive view includes:
  - `Published` column (`YYYY-MM-DD HH:MM`)
  - Rows for packages with no known vulnerabilities (`Severity = None`)
  - OpenSSL package rows derived from `security/openssl-info.txt` and NVD CPE lookups for the detected OpenSSL version

## OpenSSL runtime info sources
- `openssl-info.txt` is generated by `skills/check-security/scripts/run_security.ps1` each run.
- The web UI endpoint `/security/openssl-info` shows OpenSSL runtime details captured at app startup.
...
- Open `/security/openssl-info` in the app for startup-captured OpenSSL runtime details.
