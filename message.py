    ...
    def parse(cls, raw: bytes):
        ...
        for signer_info in cinfo['content']['signer_infos']:
            ...
            if certs and len(certs) > 0:
                ...
                # pick the exact bytes that were signed
                if 'signed_attrs' in signer_info:
                    # CMS signed‑attributes MUST be DER‑encoded SET OF Attributes
                    data_to_verify = signer_info['signed_attrs'].dump()
                else:
                    # plain “data” content
                    data_to_verify = signed_data['encap_content_info']['content'].native

                # this will raise InvalidSignature if it doesn’t match
#                pub.verify(
#                    signature,
#                    data_to_verify,
#                    asympad.PKCS1v15(),
#                    hashes.SHA256()   # or whatever hash algorithm you need
#                )



                signed_attrs = signer_info['signed_attrs']
                attrs = CMSAttributes(signed_attrs)
                attrs.tag_type = 'implicit'
                attrs.tag      = 0

                data_to_verify = attrs.dump()

                # pick the correct hash algorithm the client used:
                hash_algo = signer_info['digest_algorithm']['algorithm'].native
                hash_fn = {
                 'sha1':   hashes.SHA1(),
