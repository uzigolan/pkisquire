...
Remove-Item tests\py_*.crt, tests\py_*.key, tests\py_*.csr -ErrorAction SilentlyContinue
Write-Host ""

# Generate test credentials
Write-Host "[Prep] Generating test credentials..." -ForegroundColor Cyan
openssl genrsa -out tests\py_test.key 2048 2>&1 | Out-Null
openssl req -new -key tests\py_test.key -out tests\py_test.csr -subj "/C=IL/ST=TLV/L=Tel-Aviv/O=TestOrg/OU=IT/CN=pyscep-test.example.com" 2>&1 | Out-Null
...
.\tests\bin\pyscep.ps1 getcaps --url $SCEP_URL
Write-Host ""

# Test 2: GetCA
Write-Host "[2/3] Testing GetCA - Download CA certificate" -ForegroundColor Yellow
.\tests\bin\pyscep.ps1 getca --url $SCEP_URL -c tests\py_ca.crt
if (Test-Path tests\py_ca.crt) {
    Write-Host "      Done - CA certificate downloaded" -ForegroundColor Green
    openssl x509 -in tests\py_ca.crt -noout -subject -issuer 2>$null | ForEach-Object {
...
.\tests\bin\pyscep.ps1 enroll --url $SCEP_URL --key tests\py_test.key --csr tests\py_test.csr --ca tests\py_ca.crt --out tests\py_enrolled.crt
if (Test-Path tests\py_enrolled.crt) {
    Write-Host "      Done - Certificate enrolled successfully" -ForegroundColor Green
    $certInfo = openssl x509 -in tests\py_enrolled.crt -noout -subject -issuer -dates -serial 2>$null
...
Get-ChildItem tests\py_*.crt, tests\py_*.key, tests\py_*.csr -ErrorAction SilentlyContinue | Select-Object Name, @{N='Size';E={"{0:N0} bytes" -f $_.Length}}, LastWriteTime | Format-Table -AutoSize
