...
# --- API token helpers ---
def _enterprise_tokens_module():
    from enterprise import tokens as enterprise_tokens
...
def ensure_api_tokens_table():
    if not feature_enabled("api_tokens"):
        return
    return _enterprise_tokens_module().ensure_api_tokens_table()


def verify_api_token(raw_token: str):
    if not feature_enabled("api_tokens"):
        return None
    return _enterprise_tokens_module().verify_api_token(raw_token)
...
def api_tokens():
    if not feature_enabled("api_tokens"):
        return abort(404)
    return _enterprise_tokens_module().api_tokens_page()
...
def delete_api_token(token_id):
    if not feature_enabled("api_tokens"):
        return abort(404)
    return _enterprise_tokens_module().delete_api_token(token_id)
...
def api_tokens_state():
    if not feature_enabled("api_tokens"):
        return abort(404)
    return _enterprise_tokens_module().api_tokens_state()
